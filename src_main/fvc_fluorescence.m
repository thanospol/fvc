function U = fvc_fluorescence(DIRECTIONS,freq,r,EMT,OPTIONS,Nphoton,Etot)
%% Computes the Nphoton process fluorescence emission intensity for each direction in the far-field
...
[M] = getM2Dfluo(EMT.Er,EMT.Tr,EM.ce,Gram);

% -------------------------------------------------------------------------
% Get (Scattered) radiation intensity
% -------------------------------------------------------------------------

M.D=cmD(Etot,EMT.Er,r,M.idx,Gram,freq,Nphoton); %correlation matrix for N-photon fluoresoance
U = getU(fN,DIRECTIONS,freq,r,EMT,M,OPTIONS);

end
%% Local functions
function D=cmD(Etot,er,r,idx,Gram,freq,Nphoton)
    absorb = j_inc(Etot,er,r,idx,freq,Nphoton);
    emit=j_emm(freq);
    D=Gram*absorb.*emit;
end
function [UL] = getU(fN,DIRECTIONS,f,r,EMT,M,OPTIONS)
...
parfor ctL = 1: 2*nL
    ...
    U_fro(ctL,1) = norm(xH,'fro')^2;
end

U = U_fro(1:nL,1) + U_fro(nL+1:2*nL,1);

/**************************************************************************************************************************
 ...
 Main body of the DIRECTFN method for the evaluation of the 4-D weakly singular integrals
 over edge adjacent planar triangular elements.
...
complex<double> DIRECTFN_WS_EA_const (const double rp1[], const double rp2[], const double rp3[], const double rq1[], const double rq2[], const double rq3[], const double ko, const int Np_1D)
{
	...
	// 1. Various
        ...
        double lp_1[3], lp_2[3], lp_3[3];
        double lq_1[3], lq_2[3], lq_3[3];
        ...
        complex<double> X_plus_const, X_minus_const;
	...
	// 2. Get the weights and abscissas
	 ...
	 // for loops
         ...
         for ( int m = 1 ; m < 7 ; m++ )
            {
             ...
             for ( int n_theta = 0 ; n_theta <  Np_1D ; n_theta++ )
                {
                 ...
                 for ( int n_psi = 0 ; n_psi <  Np_1D ; n_psi++ )
                    {
                     PSI_limits ( m, THETA, &psi_A, &psi_B );
                     ...
                     B_minus = 2;
                     //
                     X_plus_const  = N_functions ( rp1, rp2, rp3, rq1, rq2, rq3, ko, 10, m, PSI, THETA, B_plus, Np_1D, z_eta, w_eta );
                     X_minus_const = N_functions ( rp1, rp2, rp3, rq1, rq2, rq3, ko, 10, m, PSI, THETA, B_minus, Np_1D, z_eta, w_eta );

                     //
                     Kernel_const = cos(PSI) * ( X_plus_const + X_minus_const );
         ...
         for ( int index = 0 ; index < 6 ; index++ )
            {
              I_const += I_subdomain_const[index];

            }
         //
         double PRECOMP = double(1.0) / double(12.0);
		 I_scalar    =  J *  I_const;
         // FINAL OUTPUT
		 return I_scalar;
    ...
    switch(argument)
        {
                case 1: // Xa
                         *theta_A = double(0.0);
                         *theta_B = M_PI / double(3.0);
                         break;
                case 2: // Xa
                         *theta_A = M_PI / double(3.0);
...
{
    double PsiA = atan( sin(theta) - sqrt(double(3.0)) * cos(theta) );
    double PsiB = atan( sin(theta) + sqrt(double(3.0)) * cos(theta) );
...
complex<double> N_functions ( const double rp1[], const double rp2[], const double rp3[], const double rq1[], const double rq2[], const double rq3[], const double ko, int basis_testing, int argument, double PSI, double THETA, int B, const int Np_1D, const double z_eta[], const double w_eta[] )
{
    ...
    complex<double> I_ETA, I_ETA_1, I_ETA_2, N;
    double ETA_1, ETA_2, ETA;
    double J_ETA, J_ETA_1, J_ETA_2;
    double ETA_a, ETA_b , ETA_1a, ETA_1b, ETA_2a, ETA_2b;

    const int N_eta = Np_1D;
     //
     double gamma_, Psi_eta, eta_theta;
     double LAMBDA_L1, LAMBDA_L2, LAMBDA_L3;
     complex<double> A_L1, A_L2, A_L3;

     switch(argument)
     {
     case 1: // Na
         gamma_ = tan(PSI) / (sin(THETA) + sqrt(double(3.0)) * cos(THETA));
         Psi_eta = (double(1.0) - gamma_) / (double(1.0) + gamma_);
         //
         I_ETA_1 = complex<double>((double)0.0 ,(double)0.0 );
         I_ETA_2 = complex<double>((double)0.0 ,(double)0.0 );
         //
         ETA_1a = double(0.0);
         ETA_1b = Psi_eta;
         ...
         ETA_2b = double(1.0);
         //
         J_ETA_1 = ( ETA_1b - ETA_1a) / double(2.0);
         J_ETA_2 = ( ETA_2b - ETA_2a) / double(2.0);

         for ( int n_eta = 0 ; n_eta <  N_eta ; n_eta++ )
         {
             ETA_1 = (( ETA_1b - ETA_1a) / double(2.0)) * z_eta[n_eta] + (( ETA_1b + ETA_1a) / double(2.0)) ;
             ETA_2 = (( ETA_2b - ETA_2a) / double(2.0)) * z_eta[n_eta] + (( ETA_2b + ETA_2a) / double(2.0)) ;
             //
             LAMBDA_L3 = ( sqrt(double(3.0)) * ( double(1.0) + ETA_1 ) ) / ( cos(PSI) * ( sin(THETA) + sqrt(double(3.0)) * cos(THETA) ) );
             A_L3 = A_functions( rp1, rp2, rp3, rq1, rq2, rq3, ko, basis_testing, PSI,THETA,ETA_1,LAMBDA_L3,B,Np_1D, z_eta, w_eta);

             LAMBDA_L2 = ( sqrt(double(3.0)) * ( double(1.0) - abs(ETA_2) ) ) / sin(PSI);
             A_L2 = A_functions( rp1, rp2, rp3, rq1, rq2, rq3, ko, basis_testing, PSI,THETA,ETA_2,LAMBDA_L2,B,Np_1D, z_eta, w_eta);
             //
             I_ETA_1 = I_ETA_1 + w_eta[n_eta] * A_L3;
             I_ETA_2 = I_ETA_2 + w_eta[n_eta] * A_L2;
         }
         I_ETA_1 = J_ETA_1 * I_ETA_1;
         I_ETA_2 = J_ETA_2 * I_ETA_2;
         //
         N = I_ETA_1 + I_ETA_2;
         break;
      ...
      case 3: // Nb
          eta_theta = sqrt(double(3.0)) / tan(THETA);
          //
          I_ETA_1 = complex<double>((double)0.0 ,(double)0.0 );
          I_ETA_2 = complex<double>((double)0.0 ,(double)0.0 );
          //
          ...
          for ( int n_eta = 0 ; n_eta <  N_eta ; n_eta++ )
          {
              ...
              LAMBDA_L1 = ( sqrt(double(3.0)) * ( double(1.0) - abs(ETA_2) ) ) / ( cos(PSI) * ( sin(THETA) - sqrt(double(3.0)) * cos(THETA) ) );
              A_L1 = A_functions( rp1, rp2, rp3, rq1, rq2, rq3, ko, basis_testing, PSI,THETA,ETA_2,LAMBDA_L1,B,Np_1D, z_eta, w_eta);
              //
              I_ETA_1 = I_ETA_1 + w_eta[n_eta] * A_L3;
              I_ETA_2 = I_ETA_2 + w_eta[n_eta] * A_L1;
          }
          I_ETA_1 = J_ETA_1 * I_ETA_1;
          I_ETA_2 = J_ETA_2 * I_ETA_2;
          //
      ...
      case 4: // Nc
          I_ETA = complex<double>((double)0.0 ,(double)0.0 );
          //
          ETA_a = double(0.0);
          ETA_b = double(1.0);
          //
          ...
          for ( int n_eta = 0 ; n_eta <  N_eta ; n_eta++ )
          {
              ...
              //
              LAMBDA_L1 = ( sqrt(double(3.0)) * ( double(1.0) - abs(ETA) ) ) / ( cos(PSI) * ( sin(THETA) - sqrt(double(3.0)) * cos(THETA) ) );
              A_L1 = A_functions( rp1, rp2, rp3, rq1, rq2, rq3, ko, basis_testing, PSI,THETA,ETA,LAMBDA_L1,B,Np_1D, z_eta, w_eta);
              //
              I_ETA = I_ETA + w_eta[n_eta] * A_L1;
          }
          I_ETA = J_ETA * I_ETA;
          //
          N = I_ETA;
      ...
      case 5: // Nd
          ...
          for ( int n_eta = 0 ; n_eta <  N_eta ; n_eta++ )
          {
              ETA = (( ETA_b - ETA_a) / double(2.0)) * z_eta[n_eta] + (( ETA_b + ETA_a) / double(2.0)) ;
              //
              LAMBDA_L2 = ( sqrt(double(3.0)) * ( double(1.0) - ETA ) ) / sin(PSI) ;
              A_L2 = A_functions( rp1, rp2, rp3, rq1, rq2, rq3, ko, basis_testing, PSI,THETA,ETA,LAMBDA_L2,B,Np_1D, z_eta, w_eta);
              //
              I_ETA = I_ETA + w_eta[n_eta] * A_L2;
          }
          I_ETA = J_ETA * I_ETA;
          //
          N = I_ETA;
...
complex<double> A_functions ( const double rp1[], const double rp2[], const double rp3[], const double rq1[], const double rq2[], const double rq3[], const double ko, int basis_testing, double PSI, double THETA, double ETA, double LAMBDA_L, int B, const int Np_1D, const double z_LAMBDA[], const double w_LAMBDA[] )
{
    ...
    for ( int n_LAMBDA = 0 ; n_LAMBDA<  N_LAMBDA ; n_LAMBDA++ )
    {
        ...
        // Simplex coordinates
        Simplex( &xi_1p, &xi_2p, &xi_3p, &xi_1q, &xi_2q, &xi_3q, LAMBDA, PSI, THETA, ETA, B);
        // Kernel
        K = Kernel( rp1, rp2, rp3, rq1, rq2, rq3, ko, basis_testing, xi_1p,xi_2p,xi_3p,xi_1q,xi_2q,xi_3q);
        // rho Jacobian
        LAMBDA_Jacobian = LAMBDA * LAMBDA;
        //
        I_LAMBDA = I_LAMBDA + w_LAMBDA[n_LAMBDA] * LAMBDA_Jacobian * K;
    }
...
void Simplex(double *xi_1p, double *xi_2p, double *xi_3p, double *xi_1q, double *xi_2q, double *xi_3q, double Lambda, double Psi, double Theta, double Eta_, int argument )
{
    ...
    *xi_3p =  xi_p / sqrt(double(3.0)) ;
    ...
    // E_Q Triangle
    double eta_q = double(0.0);
    double etaq = Lambda * cos(Psi) * cos(Theta) - Eta_;
    ...
    double xi_q = Lambda * cos(Psi) * sin(Theta);
    //
...
complex<double> Kernel ( const double rp1[], const double rp2[], const double rp3[], const double rq1[], const double rq2[], const double rq3[], const double ko, int argument, double xi_1p, double xi_2p, double xi_3p, double xi_1q, double xi_2q, double xi_3q )
{
    // Position vectors
        ...
        // basis & testing functions
            if (argument == 1)
            {
                f_g = vector_dot(lp_1,lq_1);
            }
            else if (argument == 2)
            {
                f_g = vector_dot(lp_1,lq_2);
            }
            else if (argument == 3)
            {
                f_g = vector_dot(lp_1,lq_3);
            }
            else if (argument == 4)
            {
                f_g = vector_dot(lp_2,lq_1);
            }
            else if (argument == 5)
            {
                f_g = vector_dot(lp_2,lq_2);
            }
            else if (argument == 6)
            {
                f_g = vector_dot(lp_2,lq_3);
            }
            else if (argument == 7)
            {
                f_g = vector_dot(lp_3,lq_1);
            }
            else if (argument == 8)
            {
                f_g = vector_dot(lp_3,lq_2);
            }
            else if (argument == 9)
            {
                f_g = vector_dot(lp_3,lq_3);
            }
            else if (argument == 10)
            {
                f_g = double(1.0);

...
complex<double> DIRECTFN_WS_VA_const ( const double rp1[], const double rp2[], const double rp3[], const double rq1[], const double rq2[], const double rq3[], const double ko, int Np_1D)
{
	...
	// 1. Allocate space for the arrays
         w_theta_p = new double [Np_1D];
         z_theta_p = new double [Np_1D];
         //
         w_theta_q = new double [Np_1D];
         z_theta_q = new double [Np_1D];
         //
	...
	// 2. Get the weights and abscissas
         GL_1D ( Np_1D, z_theta_p, w_theta_p );
         GL_1D ( Np_1D, z_theta_q, w_theta_q );
	 ...
	 // for loops
         ...
         I_theta_p_const = complex<double>( 0.0 , 0.0 );

         for ( int n_theta_p = 0 ; n_theta_p < Np_1D ; n_theta_p++ )
	 {
             theta_p_A = double(0.0);
             theta_p_B = M_PI / double(3.0);
             //
             ...
             //
             Lp = ( double(2.0) * sqrt(double(3.0)) ) / ( sin(THETA_p) + sqrt(double(3.0)) * cos(THETA_p) );
             //

             I_theta_q_const = complex<double>( 0.0 , 0.0 );

             for ( int n_theta_q = 0 ; n_theta_q < Np_1D ; n_theta_q++ )
             {
                 theta_q_A = double(0.0);
                 theta_q_B = M_PI / double(3.0);
                 //
                 ...
                 //
                 Lq = ( double(2.0) * sqrt(double(3.0)) ) / ( sin(THETA_q) + sqrt(double(3.0)) * cos(THETA_q) );
                 //
                 ...
                 for ( int n_psi = 0 ; n_psi < Np_1D ; n_psi++ )
                 {
                     ...
                     I_Lambda1_const = complex<double>( 0.0 , 0.0 );

                     for ( int n_Lambda1 = 0 ; n_Lambda1 < Np_1D ; n_Lambda1++ )
                     {
                         ...
                         // Kernel
                         Simplex( &xi_1p, &xi_2p, &xi_3p, &xi_1q, &xi_2q, &xi_3q, Lambda_1, PSI_1, THETA_p, THETA_q);

                         Kernel1_const = Kernel( rp1, rp2, rp3, rq1, rq2, rq3, ko, 10, xi_1p,xi_2p,xi_3p,xi_1q,xi_2q,xi_3q);
                         ...
                         //
                         I_Lambda1_const = I_Lambda1_const + w_Lambda[n_Lambda1] * F1_const;

                     } // for ( int n_Lambda1 = 0 ; Lambda1 < Np_1D ; Lambda1++ )
                     ...
                     I_Lambda2_const = complex<double>( 0.0 , 0.0 );

                     for ( int n_Lambda2 = 0 ; n_Lambda2 < Np_1D ; n_Lambda2++ )
                     {
                         ...
                         // Kernel
                         Simplex( &xi_1p, &xi_2p, &xi_3p, &xi_1q, &xi_2q, &xi_3q, Lambda_2, PSI_2, THETA_p, THETA_q);

                         Kernel2_const = Kernel( rp1, rp2, rp3, rq1, rq2, rq3, ko, 10, xi_1p,xi_2p,xi_3p,xi_1q,xi_2q,xi_3q);
                         ...
                         //
                         I_Lambda2_const = I_Lambda2_const + w_Lambda[n_Lambda2] * F2_const;
...
void Simplex(double *xi_1p, double *xi_2p, double *xi_3p, double *xi_1q, double *xi_2q, double *xi_3q, double Lambda, double Psi, double Theta_p, double Theta_q)
{
    ...
    double eta_p = Lambda * cos(Psi) * cos(Theta_p) - double(1.0);
    double xi_p  = Lambda * cos(Psi) * sin(Theta_p);
    //
    ...
    *xi_3p =  xi_p / sqrt(double(3.0)) ;
    //
    double eta_q = Lambda * sin(Psi) * cos(Theta_q) - double(1.0);
    double xi_q  = Lambda * sin(Psi) * sin(Theta_q);
    //
